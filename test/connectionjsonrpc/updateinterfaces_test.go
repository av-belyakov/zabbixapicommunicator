...
func TestUpdateInterfaces(t *testing.T) {
	...
	t.Run("Тест 2. Обновление в хосте параметра 'интерфейсы'", func(t *testing.T) {
		res, err := zc.UpdateHostParameterInterfaces(
			...
			connjsonrpc.InterfacesRequest{
				Interface: []connjsonrpc.InterfaceOptionsRequest{
					{
						Type:  1,
						Main:  1, // 1 - основной интерфейс, 0 - дополнительный
						Useip: 0,
						IP:    "27.34.110.217",
						DNS:   "dns.example.name.org",
						Port:  "10051",
						...
						// если основного интерфейса нет то добавить
						// дополнительный интерфейс нельзя, сначала
						// нужно добавить основной, затем дополнительный
					}}})
		assert.NoError(t, err)
		...
		hostInterfaces, err := zc.GetHostInterface(t.Context(), Test_HostId)
		assert.NoError(t, err)
